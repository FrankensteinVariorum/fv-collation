...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="frankenChunks" as="document-node()+" select="collection('collationChunks')"/>
    ...
    <xsl:template match="/">
        ...
        <xsl:for-each select="$collChunkIds">   
            <g id="{current()}"><!--Collation unit wrapper-->
                ...
                <xsl:variable name="SL_msColl" select="$CU_msColl//text()[not(matches(., '^\s+$'))]/string-length() => sum()"/>             
                <!--1818 data-->
   <xsl:variable name="CU_1818" as="item()" select="$frankenChunks//xml[tokenize(base-uri(), '/')[last()] => starts-with('1818')][anchor[@type='collate']/@xml:id = current()]"/> 
                <xsl:variable name="SL_1818" select="$CU_1818//text()[not(matches(., '^\s+$'))]/string-length() => sum()"/>
                <!--Thomas data -->
                ...
                <xsl:variable name="SL_Thomas" select="$CU_Thomas//text()[not(matches(., '^\s+$'))]/string-length() => sum()"/>  
                <!--1823 data -->
                ...
                <xsl:variable name="SL_1823" select="$CU_1823//text()[not(matches(., '^\s+$'))]/string-length() => sum()"/>
                <!--1831 data -->
                ...
                <xsl:variable name="SL_1831" select="$CU_1831//text()[not(matches(., '^\s+$'))]/string-length() => sum()"/>
