...
<xsl:template match="/">
    <xsl:text>1823: </xsl:text><xsl:text> &#x9;</xsl:text> <xsl:text> 1831: </xsl:text><xsl:text>&#10;</xsl:text>
    <xsl:apply-templates select="$f1823_1831Coll/descendant::app[not(@type='invariant')]"/> 
</xsl:template>
    
<xsl:template match="app[not(@type='invariant')]">
    <xsl:apply-templates select="rdg[contains(@wit, '#f1823') and not(contains(@wit, '#f1831'))]"/>
</xsl:template>
    <xsl:template match="rdg[contains(@wit, '#f1823') and not(contains(@wit, '#f1831'))]">
