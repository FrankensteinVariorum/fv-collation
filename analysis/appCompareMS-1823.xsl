...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 <!--2017-11-26 ebb: This XSLT can produce TSV outputs to compare each edition to each of the others one at a time. I've output simple collation files that collate the editions in pairs. This version is set to compare the MS to 1823.--> 

    <xsl:variable name="witnesses" select="distinct-values(//app/rdg/tokenize(@wit, ' '))"/>
    
<xsl:template match="/">
    <xsl:text>MS: </xsl:text><xsl:text> &#x9;</xsl:text> <xsl:text> 1823: </xsl:text><xsl:text>&#10;</xsl:text>
    <xsl:apply-templates select="$MS_1823Coll/descendant::app[not(@type='invariant')]"/> 
</xsl:template>
    
<xsl:template match="app[not(@type='invariant')]">
    <xsl:apply-templates select="rdg[contains(@wit, '#fMSc56') and not(contains(@wit, '#f1823'))]"/>
</xsl:template>
    <xsl:template match="rdg[contains(@wit, '#fMSc56') and not(contains(@wit, '#f1823'))]">
